...
void Rasterizer::init (const std::string & basePath, const std::shared_ptr<Scene> scenePtr, Material material, LightSource lightSource) {
	...
	// GPU resources
	initScreeQuad ();
	loadShaderProgram (basePath, material, lightSource);
	initDisplayedImage ();
...
void Rasterizer::loadShaderProgram (const std::string & basePath, Material material, LightSource lightSource) {
...
void Rasterizer::render (std::shared_ptr<Scene> scenePtr, std::vector<glm::vec3> samples) {
	const glm::vec3 & bgColor = scenePtr->backgroundColor ();
	glClearColor (bgColor[0], bgColor[1], bgColor[2], 1.f);
	glClear (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // Erase the color and z buffers.
	size_t numOfMeshes = scenePtr->numOfMeshes ();

	int sampler_size = samples.size();
	...
	for (size_t i = 0; i < numOfMeshes; i++) {
		...
		// send the Poisson Disk Samples to GPU
		size_t nb_sample = samples.size();
		for(size_t i=0; i < nb_sample; i++) {
			m_pbrShaderProgramPtr->set ("PoissonSamples[" + std::to_string(i) + "]", samples[i]);
		}
		m_pbrShaderProgramPtr->set ("sampler_size", sampler_size);
