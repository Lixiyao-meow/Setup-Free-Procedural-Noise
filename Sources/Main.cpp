...
//Material
glm::vec3 matAlbedo = glm::vec3(1.0);
float roughness = 0.2f;
float metallicness = 0.9f;

static Material material(matAlbedo, roughness, metallicness);

//LightSource
std::vector<glm::vec3> lightPos {glm::vec3(1.0, 1.0, 1.0),
								 glm::vec3(-1.0, -1.0, 2.0),
								 glm::vec3(1.0, 1.0, -1.0)};
std::vector<glm::vec3> lightIntensity {glm::vec3(3.14), glm::vec3(3.14), glm::vec3(3.14)};
std::vector<glm::vec3> lightColor {glm::vec3(0.8, 0.0, 0.2),
								   glm::vec3(0.6, 0.0, 0.8),
								   glm::vec3(0.0, 0.2, 0.7)};

float att_const = 0.0f;
float att_lin = 0.0f;
float att_quad = 0.03f;
static LightSource lightSource(lightPos, lightIntensity, lightColor,
								att_const,att_lin, att_quad);

// Noise Generator
std::vector<glm::vec3> Poisson_samples;
...
void raytrace () {
	...
	rayTracerPtr->render (scenePtr, material, lightSource);
}
...
void render () {
	if (isDisplayRaytracing)
		...
		rasterizerPtr->render (scenePtr, Poisson_samples);	
}
...
void keyCallback (GLFWwindow * windowPtr, int key, int scancode, int action, int mods) {
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_H) {
			...
			rasterizerPtr->loadShaderProgram (basePath, material, lightSource);
...
void initGLFW () {
	...
	// Create the window
	windowPtr = glfwCreateWindow (1024/2, 768/2, BASE_WINDOW_TITLE.c_str (), nullptr, nullptr);
	if (!windowPtr) {
...
void init () {
	...
	rasterizerPtr->init (basePath, scenePtr, material, lightSource); // Mut be called before creating the scene, to generate an OpenGL context and allow mesh VBOs
...
int main (int argc, char ** argv) {
	...
	PoissonDisk sampler = PoissonDisk(5, 5, 5, 0.3f);
	sampler.Samples();
	Poisson_samples = sampler.output_samples();
	
	/*
	std::ofstream file;
    file.open("samples.txt");

	for(int i=0; i<Poisson_samples.size(); i++){
		file << Poisson_samples[i][0] << " " << Poisson_samples[i][1] << " " << Poisson_samples[i][2] << '\n';
	}
	file.close();*/
