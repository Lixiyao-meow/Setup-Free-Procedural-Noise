...
public:
	...
	void init (const std::string & basepath, const std::shared_ptr<Scene> scenePtr, Material material, LightSource lightSource);
	...
	void loadShaderProgram (const std::string & basePath, Material material, LightSource lightSource);
	void render (std::shared_ptr<Scene> scenePtr, std::vector<glm::vec3> samples);
